...
API.onServerEventTrigger.connect(function (Command, args) {
    ...
    switch (Command) {
        ...
        case "ILS LSIA":
            {
                API.onUpdate.connect(function () {
                    let LSIAGlideslope = new Vector3(-1502.511, -2948.156, 13.94577);
                    let veh = API.getPlayerVehicle(API.getLocalPlayer());
                    let VehPos = API.getEntityPosition(veh);
                    let Opposite = VehPos.Z;
                    let Adjacent = LSIAGlideslope.DistanceTo2D(VehPos);
...
function LsiaL(Player, Vehicle) {
    let VehPos = API.getEntityPosition(Vehicle);
    let LSIARight = new Vector3(-1538.133, -3009.956, 13.95026);
    let LSIACenterline = new Vector3(-1545.173, -3022.796, 13.95519);
    let LSIALeft = new Vector3(-1552.572, -3035.513, 13.94475);
