...
API.onUpdate.connect(function () {
    ...
    let vehType = API.getEntitySyncedData(veh, "VehicleType");
    ...
    if (hudVisability === true) {
        ...
        if (inVeh) {
            if (vehType == "Aircraft") {
                let TNumber = API.getEntitySyncedData(veh, "Tailnumber");
                var ADFvalue = API.getEntitySyncedData(player, "ADF");
                var DMEvalue = API.getEntitySyncedData(player, "DME");
                let ADFstatus = API.getEntitySyncedData(player, "ADF Status");
                let DMEStatus = API.getEntitySyncedData(player, "DME Status");
                let ILSStatus = API.getEntitySyncedData(API.getLocalPlayer(), "ILS Status");
                let ADFID = API.getEntitySyncedData(player, "ADFID");
                let DMEID = API.getEntitySyncedData(player, "DMEID");
                let ILSID = API.getEntitySyncedData(player, "ILS ID");
                let Heading = Compass(veh);
                let TrueAirspeed = Airspeed(veh);
                let ASL = Altitude(veh);
                let vSpeed = VSpeed(veh);
                let gHeight = GroundHeight(vehPos);
                ...
                if (ILSStatus === true) {
                    if (ILSID === "LSIA") {
                        let ILSValue = API.getEntitySyncedData(player, "LSIA Glideslope");
                        let ILSCenterLine = API.getEntitySyncedData(player, "LSIA Centerline");
...
function Coordinate(Entity) {
    let pos = API.getEntityPosition(Entity);
...
function Compass(Vehicle) {
    const normalHdgVector = {
        x: API.returnNative('0x8BB4EF4214E0E6D5', 7, Vehicle),
...
function Airspeed(Vehicle) {
    let vel = API.getEntityVelocity(Vehicle);
    let Airspeedcalc = Math.sqrt(// Code by Ariana on GTA Network Forums
    vel.X * vel.X +
        vel.Y * vel.Y +
        vel.Z - vel.Z);
    let Airspeed = Math.round(Airspeedcalc * 1.94384);
    return Airspeed;
}
function Altitude(Vehicle) {
    let vehPos = API.getEntityPosition(Vehicle);
...
function VSpeed(Vehicle) {
    let vel = API.getEntityVelocity(Vehicle);
    let vSpeed = Math.sqrt(vel.X - vel.X +
        vel.Y - vel.Y +
        vel.Z * vel.Z);
